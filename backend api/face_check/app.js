...
app.post("/check-face", async (req, res) => {
  ...
  // Extracting only the label from each result object
  const labels = result.map(({ _label }) => _label);
  res.json({ result: labels }); // Sending only the labels in the response
});

// add your mongo key instead of the ***
mongoose
  .connect(
    `mongodb+srv://parthbansal2047:kavdrLvIR8Bfzrne@cluster0.rulktcu.mongodb.net/`
  )
  .then(() => {
